...
posListX = []
posListY = []
xList = [item for item in range(0,1300)] # Here 1300 its image length
prediction = False

while True:
    # Grab the image
    Success, img = cap.read()     # Read a frame from the video file.
    ...
    if posListX:
        ...
        if len(posListX) < 10 :
            ...
            # Prediction
            # X values 330 to 430 Y 590 --> from using paint in using zoom -- .this is basket size in this pic
            a = A
            b = B
            c = C - 590
    ...
    # Display
    imgContours = cv2.resize(imgContours,(0,0),None,0.7,0.7) # Resize the image to 70% of its original size.
    # cv2.imshow("Image", img) # Display the image in a window named "Image"..
    cv2.imshow("ImageColor",imgContours)
    cv2.waitKey(100) # Wait for 50 milliseconds before showing the next frame.
